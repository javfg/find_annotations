...
from utils.tools import create_tps_file
...
# Creates a HTML file with mutli-pie plot embed in it 
def create_html_file(go,kegg,min_identity,min_support,plot_file_name,name):
...
def plot(go_list, kegg_list, min_support, min_identity, name):
    ...
    kegg = create_list(kegg_list, min_support,
                       ...
                       "rgba(68, 108, 179, .6)")
    create_tps_file(go,kegg,min_support,min_identity,name)
    ...
    create_html_file(go,kegg,min_identity,min_support,plot_file_name,name)
